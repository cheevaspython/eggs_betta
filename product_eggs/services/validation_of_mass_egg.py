...
class ValidationMassEggs():
    ...
    def __init__(self, validated_data: OrderedDict):
        self.validated_data = validated_data
        self.mass_hash_map = dict()
        self.quantity_hash_map = dict()
        ...
        for egg in self.validated_data:
            if egg in self.EGGS_CATEGORY_MASS_MEANING.keys():
                if self.validated_data[egg]:
                    ...
                    self.mass_hash_map[egg] = mass
                    self.quantity_hash_map[egg] = self.validated_data[egg]
 
    def check_for_maximum_mass_in_one_truck(self):
        """
        Сравнивает общую массу с допустимой.
        """
        if sum(self.mass_hash_map.values()) > self.MAXIMUM_WEIGHT_ONE_TRUCK:
            current_mass = sum(self.mass_hash_map.values())
    ...
    def check_for_capacity_in_boxes(self):
        ...
        """
        for cat, quantity in self.quantity_hash_map.items():
