  ...
  <form action="" style="width: 1000px; height: 95vh;">
    <div class="modal-card" style="width: 900px">
      <header class="modal-card-head" style="background-color: #c3c3c3; text-align: center">
        <p class="modal-card-title" style="color: whitesmoke; font-size: 30px; font-weight: 900">Внесение входящего платежа по Форме 2</p>
        <button
          type="button"
          class="delete"
          @click="$emit('close')"/>
      ...
      <section class="modal-card-body is-fullwidth">
        <div class="flexbox__row flexbox__space-between input-border">
          <div style="margin: 0 10px">Покупатель</div>
          <b-field>
...
<script>
export default {
  name: "ModalPaymentCashForm",
  props: ['trader', 'buyer'],
  data() {
    return {
      name: this.buyer.name,
      inn: this.buyer.inn,
      payLimit: this.buyer.pay_limit,
      debt: this.buyer.balance_form_two < 0 ? Math.abs(this.buyer.balance_form_two) : 0 ,
      docNumber: null,
      encroachment: null,
      comment: null,
      loading: false,
  ...
  created() {
    if (this.buyer.debt_deals) {
      this.debtDeals = this.buyer.debt_deals.filter(el => el.cash == true)
    ...
    async entryPayment() {
      ...
      if (this.universalPayment) {
        ...
        if (remainder > 0) {
          ...
          tmp_json_for_multi_pay_order.tail_form_two = remainder
          if (deals.length > 0) {
        ...
        this.encroachment = 0
        const payloadMulty = [formDataMulty, this.buyer.documents_contract_id]
        await this.$store.dispatch('ca/buyerCashDocs', payloadMulty)
      }
      else {
        const dealId = this.dealsOnPayment[0].id
