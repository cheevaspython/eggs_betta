  ...
  <form action="" style="width: 1000px">
    <div class="modal-card" style="width: 900px; height: 95vh">
      <header class="modal-card-head" style="background-color: #c3c3c3; text-align: center">
        <p class="modal-card-title" style="color: whitesmoke; font-size: 30px; font-weight: 900">Создание заявки от покупателя</p>
        <button
          type="button"
          class="delete"
          @click="$emit('close')"/>
      ...
      <section class="modal-card-body is-fullwidth flexbox flexbox__column">
        <div class="categories__content">
          ...
          <div v-for="row in selectedCategories" class="flexbox__row flexbox__space-between" style="width: 100%;">
            ...
            <b-field label="Количество (дec.)" class="mb-1">
          ...
          <b-button class="button is-success is-light mr-2" icon-left="plus" @click="addRow" rounded :disabled="addBtnDisabled">Добавить категорию</b-button>
          <b-button class="button is-success is-light" icon-left="minus" @click="removeRow" rounded :disabled="removeBtnDisabled">Убрать категорию</b-button>
          ...
          <div class="flexbox__row flexbox__space-between table">
            ...
            <div class="flexbox flexbox__column" style="padding: 10px; width: 423px; border-left: solid 2px #ebebeb;">
                ...
                <b-dropdown :triggers="['hover']" aria-role="list" style="width: 100%; margin-bottom: 10px;">
                  <template #trigger>
                    <b-button
                      ...
                      label="Выберите адрес разгрузки"
              ...
              <div class="flexbox flexbox__column" style="gap: 5px;">
                <div class="flexbox__row flexbox__space-between input-border" style="width: 100%; margin-top: 10px">
...
export default {
  ...
  data() {
    return {
      ...
      dirt: 0,
      dirt_cost: 0,
      warehouseAddress1: this.trader.warehouse_address_1,
      warehouseAddress2: this.trader.warehouse_address_2,
      warehouseAddress3: this.trader.warehouse_address_3,
      warehouseAddress4: this.trader.warehouse_address_4,
      warehouseAddress5: this.trader.warehouse_address_5,
      unloading_address: '',
      dates: null,
      postponementPay: null,
      selectedCategories: [{
        title: 'CB',
        count: 0,
        price: 0,
  ...
  methods: {
    ...
    async sendBuyerOrder() {
      ...
      if (!this.unloading_address) {
        return alert('Не выбран адрес склада')
      }
      ...
      const data = {
        ...
        postponement_pay: this.postponementPay ? this.postponementPay : 0,
        region: this.trader.region
      }
      this.loading = true
      const success = await this.$store.dispatch('eggs/postBuyerApp', data)
