...
class BalanceBuyerWs(GenericAsyncAPIConsumer):
    ...
    async def balance_buyer_activity(
        ...
        if self.auth:
            for request_id in subscribing_request_ids:
                await self.reply(data=await self.get_balance_buyer('get_balance_buyer'), action=action, request_id=request_id)
        else:
            await super().close() 
    ...
    @action()
    async def subscribe_to_balance_buyer(self, request_id: str, **kwargs):
        if self.auth:
