...
class DocumentsDealModel(models.Model):
    ...
    edo_seller_documents = models.BooleanField(
        editable=True, default=False,
        verbose_name='Документооборот с продавцом по ЭДО',
    )
    edo_buyer_documents = models.BooleanField(
        editable=True, default=False,
        verbose_name='Документооборот с покупателем по ЭДО',
    )
    edo_logic_documents = models.BooleanField(
        editable=True, default=False,
        verbose_name='Документооборот с переозчиком по ЭДО',
    )
    ...
    payment_for_contract = models.BooleanField(
        editable=True, default=False,
        verbose_name='Оплата по основному договору',
    )
    ...
    UPD_outgoing_signed = models.FileField(
        blank=True, null=True,
        upload_to=(f'uploads/deal_docs/UPD_outgoing/{datetime.today().year}/{datetime.today().month}/' +
            f'{datetime.today().day}/{datetime.today().hour}-{datetime.today().minute}-{datetime.today().second}/'),
        verbose_name='Исходящая УПД подписаная',
    )
    ...
    international_deal_TTN_seller = models.FileField(
        blank=True, null=True,
        upload_to=(f'uploads/deal_docs/international_deal_TTN/{datetime.today().year}/{datetime.today().month}/' +
            f'{datetime.today().day}/{datetime.today().hour}-{datetime.today().minute}-{datetime.today().second}/'),
        verbose_name='Международная сделка, ТТН покупка',
    )
    international_deal_TTN_buyer = models.FileField(
        blank=True, null=True,
        upload_to=(f'uploads/deal_docs/international_deal_TTN/{datetime.today().year}/{datetime.today().month}/' +
            f'{datetime.today().day}/{datetime.today().hour}-{datetime.today().minute}-{datetime.today().second}/'),
        verbose_name='Международная сделка, ТТН продажа',
    )
